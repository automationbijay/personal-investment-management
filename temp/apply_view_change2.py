...
view_sql = """
DROP VIEW IF EXISTS public.view_mf_ask_bid CASCADE;

CREATE OR REPLACE VIEW public.view_mf_ask_bid 
...
AS
 WITH config AS (
         ...
         SELECT nav."MF" AS symbol,
            ...
            w.wacc_rate::numeric AS wacc_rate,
            w.quantity AS my_quantity,
            avg.ltp_avg_5d,
            avg.volume_avg_5d,
            avg.vwap_avg_5d
           FROM view_mf_summary_analytics nav
             JOIN md_parsed p ON nav."MF" = p.symbol::text
             CROSS JOIN config c
             LEFT JOIN wiki_profit_loss_analysis w ON nav."MF" = w.symbol
             LEFT JOIN wiki_average avg ON nav."MF" = avg.symbol
        )
 SELECT symbol,
    adjusted_nav,
    mf_ltp AS ltp,
...
print("View view_mf_ask_bid updated successfully!")
