        ...
        BEGIN
            ...
            IF v_actual_date IS NOT NULL AND v_actual_date < v_nav_date THEN
                INSERT INTO public.raw_price_history (
                    "symbol-with-date", symbol, "Date", "LTP", "Open", "High", "Low", "Close", "Volume"
                ) 
                VALUES (
                    NEW.symbol || '-' || TO_CHAR(v_nav_date, 'YYYY-MM-DD'),
            ...
            ELSIF v_actual_date IS NULL THEN
                ...
                -- We use today's LTP as a fallback (or 0 if literally no price history exists at all)
...
try:
    cur.execute(sql)
    conn.commit()
    print("Fixed trigger for zero-history edge case!")
